...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  for PROXY_URL in "${GITHUB_PROXY[@]}"; do
    PROXY_STATUS_CODE=$(wget --server-response --spider --quiet --timeout=3 --tries=1 ${PROXY_URL}https://github.com/lgpay/Argo-Nezha-Service-Container/raw/main/README.md 2>&1 | awk '/HTTP\//{last_field = $2} END {print last_field}')
  ...
  if [ "$REVERSE_PROXY_MODE" = 'grpcwebproxy' ]; then
    wget -c ${GH_PROXY}https://github.com/lgpay/Argo-Nezha-Service-Container/releases/download/grpcwebproxy/grpcwebproxy-linux-$ARCH.tar.gz -qO- | tar xz -C $WORK_DIR
  ...
  if [ -z "$DASHBOARD_VERSION" ]; then
    ...
    if [ "$VERSION_NUM" = "0.20.13" ]; then
      ...
      if wget -q --spider ${GH_PROXY}https://github.com/lgpay/nezha-dashboard/releases/download/v0.20.13-lts/dashboard-linux-$ARCH.zip 2>/dev/null; then
        wget -O /tmp/dashboard.zip ${GH_PROXY}https://github.com/lgpay/nezha-dashboard/releases/download/v0.20.13-lts/dashboard-linux-$ARCH.zip
      else
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/lgpay/Argo-Nezha-Service-Container/raw/main/sqlite.db
