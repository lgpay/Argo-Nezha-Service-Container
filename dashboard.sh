...
dashboard_variables() {
  ...
  if [ -z "$DASHBOARD_VERSION" ]; then
    ...
    if [ "$VERSION_NUM" = "0.20.13" ]; then
      ...
      if wget -q --spider ${GH_PROXY}https://github.com/lgpay/nezha-dashboard/releases/download/v0.20.13/dashboard-linux-$ARCH.zip 2>/dev/null; then
        { wget -qO $TEMP_DIR/dashboard.zip ${GH_PROXY}https://github.com/lgpay/nezha-dashboard/releases/download/v0.20.13/dashboard-linux-$ARCH.zip >/dev/null 2>&1; }&
      else
